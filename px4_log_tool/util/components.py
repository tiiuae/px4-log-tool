...
def resample_unified(
    ...
    """
    reference: pd.DataFrame = deepcopy(msg_reference)
    num_labels: list[str] = []
    cat_labels: list[str] = []
    mission_names: list[str] = sorted(unified_df["mission_name"].unique())
    ...
    for mission in mission_names:
        merged_df: pd.DataFrame = unified_df[unified_df["mission_name"] == mission]
        for label in merged_df.columns:
            ...
            if msg[-1].isdigit():
                msg = msg[:-1]
            label_dc: pd.DataFrame = reference[reference["Alias"] == f"{msg}_{param}"]
            label_dc = label_dc.reset_index(drop=True)
            dc: pd.Series = label_dc["Dataclass"]
            if dc.size < 1:
                continue
...
def get_msg_reference(verbose: bool = False) -> Optional[pd.DataFrame]:
    import importlib.resources

    try:
        ref = importlib.resources.files("px4_log_tool").joinpath("msg_reference.csv")
        with importlib.resources.as_file(ref) as csv_path:
            msg_reference: pd.DataFrame = pd.read_csv(csv_path)
...
def dump_template_filter(file_path: str, verbose: bool = False) -> None:
    """
    ...
    filter_path: str = os.path.join(file_path, "filter.yaml")
    try:
        with open(filter_path, "w") as f:
            yaml.dump(
                template_data, f, sort_keys=False, indent=2, default_flow_style=False
            )
        log(
            f"Template filter file created at '{filter_path}'",
            verbosity=verbose,
            log_level=0,
        )
    except IOError as e:
        log(
            f"Error writing template file to {filter_path}: {e}",
            verbosity=verbose,
            log_level=1,
        )


def extract_filter(filter_str: Optional[str], verbose: bool = False) -> Dict[str, Any]:
    """
    ...
    config_source_name: str = (
        ...
        if filter_str
        else "default configuration (no file provided)"
    )
    ...
    if isinstance(filter_str, str):
        ...
        try:
            ...
            log(
                f"Warning: Filter file {config_source_name} not found.",
                verbosity=verbose,
                log_level=1,
            )
            log(
                "Using default values for all sections.", verbosity=verbose, log_level=1
            )
            if verbose:
            ...
            log(
                f"Error parsing YAML file {config_source_name}: {e}.",
                verbosity=verbose,
                log_level=1,
            )
            log(
        ...
        log(
            f"Warning: Invalid filter_file_path type ({type(filter_str)}). Expected string or None.",
            verbosity=verbose,
            log_level=1,
        )
    ...
    final_filter_config: Dict[str, Any] = {}

    for key, schema_item in DEFAULT_FILTER_CONFIG.items():
        default_value_for_key: Any = deepcopy(schema_item["default"])
        description: str = schema_item["description"]

        section_value_to_assign: Any = None

        if key not in user_config:
            log(
                f"Warning: Missing section '{key}' in {config_source_name}.",
                verbosity=verbose,
                log_level=1,
            )
            log(
                "Using default values for this section.", verbosity=verbose, log_level=1
            )
            log("", verbosity=verbose, log_level=1)
            section_value_to_assign = default_value_for_key
        else:
            user_section_data: Any = user_config[key]
            if isinstance(default_value_for_key, dict):
                if not isinstance(user_section_data, dict):
                    log(
                        f"Warning: Section '{key}' in {config_source_name} is not a dictionary as expected. User provided type: {type(user_section_data)}.",
                        verbosity=verbose,
                        log_level=1,
                    )
                    log(
                        "Using default values for this entire section.",
                        verbosity=verbose,
                        log_level=1,
                    )
                    log("", verbosity=verbose, log_level=1)
                    ...
                    merged_section: Dict[str, Any] = deepcopy(default_value_for_key)

                    for user_sub_key in user_section_data:
                        if (
                            user_sub_key not in merged_section
                        ):  # Check against keys in the default dict
                            log(
                                f"Info: User-provided sub-key '{user_sub_key}' in section '{key}' of {config_source_name} is not defined in the default schema for this section. It will be included.",
                                verbosity=verbose,
                                log_level=0,
                            )
                    ...
                    defaulted_sub_keys_this_section: list[str] = []
                    for default_sub_k in (
                        default_value_for_key
                    ):  # Iterate over keys in the original default
                        if (
                            default_sub_k not in user_section_data
                        ):  # Check if user provided it
                            defaulted_sub_keys_this_section.append(default_sub_k)

                    if defaulted_sub_keys_this_section:
                        log(
                            f"Info: For section '{key}' in {config_source_name}, default values were applied for the following missing sub-key(s): {', '.join(defaulted_sub_keys_this_section)}.",
                            verbosity=verbose,
                            log_level=0,
                        )
                        log("", verbosity=verbose, log_level=1)
        ...
        current_section_data: Any = final_filter_config[key]
...
def get_ulog_files(ulog_dir: str, verbose: bool = False) -> list[tuple[str, str]]:
    ...
    Returns:
    - list[tuple[str, str]]: A list of tuples, where each tuple contains the file path and filename.
    """
    ...
    log(
        msg=f"Converting [{len(ulog_files)}] .ulog files.",
        verbosity=verbose,
        log_level=0,
    )
    return ulog_files
...
def get_csv_dirs(csv_dir: str, verbose: bool = False) -> list[str]:
    ...
    log(
        msg=f"Converting [{len(csv_dirs)}] .csv directories.",
        verbosity=verbose,
        log_level=0,
    )
    return csv_dirs
...
def convert_dir_ulog_csv(
    ...
    Args:
    - ulog_files (list[tuple[str, str]]): A list of tuples, where each tuple contains the file path and filename.
...
def convert_dir_csv_db3(
    csv_dirs: list[str],
    output_dir: str,
    topic_prefix: str,
    capitalise_topics: bool,
    verbose: bool = False,
) -> None:
...
def adjust_topics(
    directory_address: str, filter: Dict[str, Any], verbose: bool = False
) -> None:
